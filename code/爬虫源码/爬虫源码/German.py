...
list=[30,21,1,11,20,11,14,8,6,1,14,7,10,6,3,9,1,9,27,7,6,18,9,]
def get_content(url):
...
cursor = conn.cursor() 
cursor.execute('CREATE TABLE `GermanWord` (`id` int NOT NULL,                `word` varchar(200) DEFAULT NULL,               `translation` varchar(1000) DEFAULT NULL,       PRIMARY KEY (`id`)          ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4')
conn.commit()
...
while(i<len(allContent)):
    ...
    cursor.execute('INSERT INTO GermanWord (id,word,translation)       VALUES (%s,%s,%s)' ,(i,allContent[i],allChinContent[i]))
    i=i+1
    print("make"+str(i))
