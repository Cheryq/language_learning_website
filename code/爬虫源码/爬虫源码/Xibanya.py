...
list=[25,6,28,15,19,8,6,8,7,10,1,6,14,5,5,23,1,13,13,11,2,7,1,1,1]
def get_content(url):
...
for i in range(0,len(list)):
    for j in range(0,list[i]):
        ...
        url='http://sp.qsbdc.com/word/word_list.php?letter_id='+str(i+1)+'&&tag=all&&page_id='+str(j+1)
        get_content(url)
    print("i"+str(i)+"done")
print(allContent)
...
cursor = conn.cursor() 
cursor.execute('CREATE TABLE `Xbanya` (`id` int NOT NULL,                `word` varchar(200) DEFAULT NULL,               `translation` varchar(1000) DEFAULT NULL,       PRIMARY KEY (`id`)          ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4')
conn.commit()
...
while(i<len(allContent)):
    ...
    cursor.execute('INSERT INTO Xbanya (id,word,translation)       VALUES (%s,%s,%s)' ,(i,allContent[i],allChinContent[i]))
    i=i+1
    print("make"+str(i))
